...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    if settings.KAFKA_BOOTSTRAP_SERVERS:
        ...
        from .infrastructure.orm_models import PaymentOutboxEventORM

        app.state.outbox = GenericOutboxWorker(
            app.state.session_factory,
            app.state.publisher,
            PaymentOutboxEventORM,
            default_topic=settings.PAYMENT_EVENTS_TOPIC,
            batch_size=100,
...
def create_app() -> FastAPI:
    app = FastAPI(
        title="SafarPay Payment Service",
        description="Wallets, payment intents, commission reservations, provider callbacks, and financial ledger.",
        version="1.0.0",
        lifespan=lifespan,
    )

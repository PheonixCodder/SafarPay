...
class SDriverLicenseRegistrationController extends GetxController {
  SDriverLicenseRegistrationController({
  ...
  String? get expiryError => SValidator.validateFutureDate(
        expiryDate.value,
  ...
  Future<bool> submit() async {
    ...
    if (!isValid || dateError != null) {
      errorMessage.value = dateError ?? 'Please check your license details.';
      return false;
    }
    if (!hasAllImages) {
      errorMessage.value = 'Upload front and back license images.';
      return false;
    }

    isSubmitting.value = true;
    try {
      final response = await _repository.submitLicense(
      ...
      await _upload(response, 'license_front', frontImage.value!);
      await _upload(response, 'license_back', backImage.value!);
      return true;
    } catch (error) {
